...
// --- Menu & Stage Constants ---
#define NUM_MAIN_MENU_ITEMS 3
#define NUM_TRAINING_STAGES 8

// --- Milliways Member & Fitness/Exercise Constants ---
...
// --- Milliways Member & Exercise Data ---
...
void display_member_list(); // Bonus function

/**
 * Main function to run the top-level menu loop.
 */
int main() {
    char training_status[NUM_TRAINING_STAGES] = {0};
    float health_scores[NUM_MEMBERS][NUM_TESTS] = {{0.0f}};
    // 3D array: member -> day -> routine string
...
/**
 * (Bonus) Shared function to display the list of Milliways members.
 */
...
void handle_training_menu(char status[], float scores[][NUM_TESTS], char routines[][NUM_DAYS][MAX_ROUTINE_LEN]) {
    // ... (This function remains mostly the same, but now passes routines down)
    char input_buffer[100];
    int choice;
    while (1) {
...
void getHealth(const float scores[][NUM_TESTS]) {
    display_member_list(); // Calling shared function
}
void setHealth(float scores[][NUM_TESTS]) { /* ... Same as previous ... */ }

int is_routine_valid(char* routine_str, int* core_used_this_week) {
    char buffer[MAX_ROUTINE_LEN];
    strcpy(buffer, routine_str); // Use a copy for strtok
    ...
    // If routine is valid and used a core exercise, update the weekly flag
    if (uses_core_today) {
...
/**
 * Sets the weekly workout routine for each Milliways member.
 */
void setExerciseRoutine(char routines[][NUM_DAYS][MAX_ROUTINE_LEN]) {
    ...
    for (int i = 0; i < NUM_MEMBERS; i++) {
        int core_used_this_week = 0; // Reset flag for each member
...
/**
 * (Bonus) Displays the workout routine for a specific member.
 */
void getExerciseRoutine(const char routines[][NUM_DAYS][MAX_ROUTINE_LEN]) {
